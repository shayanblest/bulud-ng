...
1. **Product intake (for a new feature/component request without an issue ID)** — Route the request to the Technical Product Manager. The manager checks duplicates, defines scope and acceptance criteria, creates the Beads task, assigns `role:senior-angular-developer`, and reports the task ID. The user does not need to create the task manually.
...
3. **Plan** — Select the PM- or tester-created Beads issue and confirm scope, acceptance criteria, affected states, and validation requirements.
...
5. **Self-validate and hand off** — Run the narrowest relevant checks, then the required Definition of Done checks from `AGENTS.md`. Update the Beads issue with changed files and results. When validation passes, automatically post the review request below without asking the user for confirmation. Keep the issue `in_progress`.
6. **Automatic review** — The review-request comment triggers the Strict Code Reviewer automatically, without a user prompt. The reviewer checks the same Beads issue's diff, tests, public API, accessibility, configuration precedence, theming, and dependency changes.
7. **Resolve** — If the reviewer finds a blocker, return the issue to the developer with concrete findings. Repeat validation and review until no blockers remain.
8. **Automatic close** — When review approval and required checks pass, the reviewer automatically comments approval and closes the same Beads issue. No user confirmation is requested. Blockers leave the issue `in_progress` and automatically return findings to the developer.
...
bd ready
bd show <issue-id>
bd create --type feature --title "..." --description "..." --acceptance "..." --labels "role:senior-angular-developer"
...
The external AI runner must route feature and component requests without an issue ID to the Technical Product Manager. The manager creates and scopes the Beads task before implementation, so users can submit requests in natural language without manually running Beads commands.
