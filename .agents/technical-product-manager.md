...
| E3-T4 | Run final strict review and release validation | `type:task`, `area:review`, `priority:P0` | E0-T3, E3-T2, E3-T3 | Reviewer approves the remote PR; exactly one commit remains; all required CI checks pass; no API/dependency risks remain. |
...
- Do not close a Beads or GitHub issue until its PR is merged and the merge reference is recorded.
